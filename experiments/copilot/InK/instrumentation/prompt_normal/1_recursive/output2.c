...
// Application initialization: create and activate thread
void __app_init() {
    __CREATE(1, t_init);   // Create thread with priority 1 and entry task t_init
    __SIGNAL(1);           // Activate the thread immediately after flashing
}

// Optional reboot hook (can be left empty or used if needed)
void __app_reboot() {}
