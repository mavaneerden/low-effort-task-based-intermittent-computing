...
// Global variables (automatically task-shared by CITID compiler)
...
// Helper function (used inside a task, so its variables are task-shared)
void increment_other_variable() {
    other_variable++;
}

// Define the CITID thread with priority 10, activated on first boot
