...
CITID_PERSISTENT uint16_t adc_value_isr = 0;
citid_event_t adc_event = { .data = &adc_value_isr, .size = sizeof(uint16_t), .timestamp = 0 };
...
void __attribute__ ((interrupt(ADC_VECTOR))) ADC_ISR(void) {
    adc_value_isr = ADCMEM0; // Read ADC result
