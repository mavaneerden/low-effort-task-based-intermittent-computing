...
__shared(
    ...
    _q15 data_array[N_SAMPLES]; uint8_t pinCont; uint16_t _v_pinState;
    unsigned _v_seed; unsigned _v_count; unsigned _v_discardedSamplesCount;
    unsigned _v_totalCount; unsigned _v_movingCount;
    unsigned _v_stationaryCount; unsigned _v_trainingSetSize;
    unsigned _v_samplesInWindow; accelReading _v_window[ACCEL_WINDOW_SIZE];
    features_t _v_features; features_t _v_model_stationary[MODEL_SIZE];
    features_t _v_model_moving[MODEL_SIZE]; class_t _v_class;
    run_mode_t _v_mode;)
...
void ACCEL_singleSample_(threeAxis_t_8 *result) {
  ...
  result->x =
      (((int16_t)acc_data[1]) << 8) | acc_data[0]; //(__GET(_v_seed)*17)%85;
  result->y =
      (((int16_t)acc_data[3]) << 8) | acc_data[2]; //(__GET(_v_seed)*17*17)%85;
  result->z = (((int16_t)acc_data[5]) << 8) | acc_data[4];
  ; //(__GET(_v_seed)*17*17*17)%85;
}
...
TASK(task_transform) {
  ...
  for (i = 0; i < ACCEL_WINDOW_SIZE; i++) {
    if (window[i].x < SAMPLE_NOISE_FLOOR || window[i].y < SAMPLE_NOISE_FLOOR ||
...
TASK(task_featurize) {
  ...
  for (i = 0; i < ACCEL_WINDOW_SIZE; i++) {
    stddev.x +=
        window[i].x > mean.x ? window[i].x - mean.x : mean.x - window[i].x;
    stddev.y +=
        window[i].y > mean.y ? window[i].y - mean.y : mean.y - window[i].y;
    stddev.z +=
        window[i].z > mean.z ? window[i].z - mean.z : mean.z - window[i].z;
  }
  stddev.x >>= 2;
  stddev.y >>= 2;
  stddev.z >>= 2;
...
TASK(task_classify) {
  ...
  for (ios_i = 0; ios_i < MODEL_SIZE; ios_i++) {
    model_stationary[ios_i] = __GET(_v_model_stationary[ios_i]);
  }

  features_t model_moving[MODEL_SIZE];
  for (ios_i = 0; ios_i < MODEL_SIZE; ios_i++) {
    model_moving[ios_i] = __GET(_v_model_moving[ios_i]);
  }
  ...
  for (i = 0; i < MODEL_SIZE; ++i) {
    long int stat_mean_err = ((model_stationary[i].meanmag) > meanmag)
                                 ? ((model_stationary[i].meanmag) - meanmag)
                                 : (meanmag - model_stationary[i].meanmag);

    long int stat_sd_err = ((model_stationary[i].stddevmag) > stddevmag)
                               ? ((model_stationary[i].stddevmag) - stddevmag)
                               : ((stddevmag - model_stationary[i].stddevmag));

    long int move_mean_err = ((model_moving[i].meanmag) > meanmag)
                                 ? ((model_moving[i].meanmag) - meanmag)
                                 : (meanmag - model_moving[i].meanmag);

    long int move_sd_err = ((model_moving[i].stddevmag) > stddevmag)
                               ? ((model_moving[i].stddevmag) - stddevmag)
                               : ((stddevmag - model_moving[i].stddevmag));
