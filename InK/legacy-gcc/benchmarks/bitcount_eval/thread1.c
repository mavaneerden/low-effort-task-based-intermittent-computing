...
__shared(
    ...
    uint8_t pinCont; unsigned n_0; unsigned n_1; unsigned n_2; unsigned n_3;
    unsigned n_4; unsigned n_5; unsigned n_6; unsigned function;
    unsigned iteration; uint32_t seed;)
...
TASK(t_BW_btbl_bitcount) {
  ...
  n_4 += bits[U.ch[0]] + bits[U.ch[1]] + bits[U.ch[3]] + bits[U.ch[2]];
  uint32_t tmp_seed = seed;
  seed = tmp_seed + 13;
  iteration++;
