...
__shared(uint16_t status; int16_t array_X[N_SAMPLES];
         int16_t array_Y[N_SAMPLES]; int16_t array_Z[N_SAMPLES];
         int16_t minFeatureZ; int16_t maxFeatureZ; uint16_t meanFeature;
         uint16_t stddevFeature; uint16_t ACT1_count; uint16_t ACT2_count;
         uint16_t ACT3_count;)
...
TASK(t_features) {
  ...
  unsigned stddevmag =
      stddevX * stddevX + stddevY * stddevY + stddevZ * stddevZ;
...
TASK(t_distance) {
  ...
  uint16_t ACT1_eudistance =
      isqr_Newton(ipow(ACT1_trained_maxZ - __GET(maxFeatureZ)) +
  ...
  uint16_t ACT2_eudistance =
      isqr_Newton(ipow(ACT2_trained_maxZ - __GET(maxFeatureZ)) +
  ...
  uint16_t ACT3_eudistance =
      isqr_Newton(ipow(ACT3_trained_maxZ - __GET(maxFeatureZ)) +
