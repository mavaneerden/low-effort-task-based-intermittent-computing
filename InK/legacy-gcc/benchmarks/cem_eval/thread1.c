...
__shared(
    ...
    uint8_t pinCont; unsigned _v_letter_idx; letter_t _v_letter;
    sample_t _v_prev_sample; index_t _v_out_len; index_t _v_node_count;
    sample_t _v_sample; index_t _v_sample_count; index_t _v_sibling;
    index_t _v_child; index_t _v_parent; index_t _v_parent_next;
    node_t _v_parent_node; node_t _v_sibling_node; index_t _v_symbol;
    node_t _v_compressed_data[BLOCK_SIZE]; node_t _v_dict[DICT_SIZE];
...
TASK(task_letterize) {
  ...
  letter_t letter =
      (__GET(_v_sample) & (LETTER_MASK << letter_shift)) >> letter_shift;
