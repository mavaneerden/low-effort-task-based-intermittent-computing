...
__shared(uint8_t pinCont; uint16_t _v_pinState;
         // unsigned _v_seed;
         unsigned _v_count; unsigned _v_discardedSamplesCount;
...
TASK(task_train) {
  ...
  switch (__GET(_v_class)) {
  case CLASS_STATIONARY:
    __SET(_v_model_stationary[trainingSetSize].meanmag,
          __GET(_v_features.meanmag));
    __SET(_v_model_stationary[trainingSetSize].stddevmag,
          __GET(_v_features.stddevmag));
    break;
  case CLASS_MOVING:
    __SET(_v_model_moving[trainingSetSize].meanmag, __GET(_v_features.meanmag));
    __SET(_v_model_moving[trainingSetSize].stddevmag,
          __GET(_v_features.stddevmag));
    break;
  }
