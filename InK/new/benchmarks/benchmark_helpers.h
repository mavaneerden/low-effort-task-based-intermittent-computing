...
#define _interrupt(signame) __attribute__((interrupt(signame))) void isr_##signame()

/* defines an interrupt routine which allows nested interrupts */
#define __nested_interrupt(signame) __attribute__((interrupt(signame), signal, wakeup)) void nested_isr_##signame()
...
#define __mcu_gpio_unlock() PM5CTL0 &= ~LOCKLPM5
